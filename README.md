...
The print output from bpftrace should be tuples (in JSON format e.g. `-f json`) where the first item in the tuple is the event type and the rest of the items are key value pairs e.g. `("track_event", "name", "page_fault_user", "type", "END")`
...
## Track Descriptor
These define track names in the Perfetto UI. At the moment only "track_event" event types can utilize custom tracks and to do that they need to set the track name in your track event e.g. `(..., "track_name", "Sub Parent A")`.
...
### thread_name
These are specifically for naming pid/tid tracks and require both the "pid" and "tid" pairs.
